...
if [ "$renew_cert" == "no" ]; then
    ...
    "$letsencrypt" certonly --dns-cloudflare --dns-cloudflare-credentials "$credentials" --agree-tos --text --expand --manual-public-ip-logging-ok --email "$letsencrypt_email" -d "$CN"  || {
